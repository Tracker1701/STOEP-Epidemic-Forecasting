...
class ModelTrainer(object):
    ...
    def get_model(self):
        if self.params['model'] == 'STOEP':
            model = STOEP.stoep(num_nodes = self.params['N'], in_dim=4, blocks=2, layers=3,
                                    in_len=self.params['obs_len'],
    ...
    def evaluate(y_pred: np.array, y_true: np.array):
        ...
        def sMAPE(y_pred: np.array, y_true: np.array):
        ...
        print('sMAPE:', round((sMAPE(y_pred, y_true).item() * 100), 4), '%')
        print('RAE:', round(RAE(y_pred, y_true).item(), 4))
        return MSE(y_pred, y_true), RMSE(y_pred, y_true), MAE(y_pred, y_true), sMAPE(y_pred, y_true), RAE(y_pred,
                                                                                                          y_true)


def smape(preds, labels, eps=1e-5):
    denominator = (torch.abs(labels) + torch.abs(preds)) / 2.0
    loss = torch.abs(preds - labels) / (denominator + eps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', '--model', type=str, help='Specify model', choices=['STOEP'], default='STOEP')
